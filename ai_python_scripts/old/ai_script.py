...
def collect_tracker_data(
    s_bots: dict[np.int64, tf.TfBot],
...
def send_tensor_angles(
    player_input_messages: Queue, angles: torch.Tensor, s_bots: dict[np.int64, tf.TfBot]
):
...
def collect_data_file_training(
    generated_angles: torch.Tensor,
    proper_angles: torch.Tensor,
    rewards: torch.Tensor,
    overall_evaluation_tracker,
    accuracy_tracker,
):
...
def from_file_training_loop(
    ...
    collect_data_file_training(
        generated_angles, proper_angles, rewards, overall_evaluation_tracker, accuracy_tracker
    )
...
def ddpg_training_loop(
    ...
    states, proper_angles = (
        file_replay_buffer.sample_from_cluster()
    )  # shape: (batch, 6), (batch, 2)
    ...
    rewards = evaluate_decision_with_angles_DDPG(
        predicted_angles, proper_angles
    ...
    # === TRACKING ===
    collect_data_file_training(
        actions, proper_angles, rewards, overall_evaluation_tracker, accuracy_tracker
    )


def in_game_training_loop(
    bots: dict[np.int64, tf.TfBot],
    player_input_messages,
    actor: ai.Actor,
    actor_optimizer: ai.optim,
    ...
    should_restart, restart_count = send_and_wait_for_positions(
        restart_count, player_input_messages
    )
    if should_restart:
        return
    ...
    should_restart, restart_count = send_and_wait_for_damage_data(
        restart_count, player_input_messages
    )
    if should_restart:
        return
...
def generate_data_loop(player_input_messages, bots, replay_buffer: ai.ReplayBuffer):
    ...
    for i, s_bot in enumerate(s_bots.values()):
        pitch, yaw = math_magic(
            s_bot.pos_x, s_bot.pos_y, s_bot.pos_z, t_bot.pos_x, t_bot.pos_y, t_bot.pos_z
        )
...
def main():
    ...
    user_listener = threading.Thread(
        target=user_input_listener, args=(player_input_messages,), daemon=True
    )
